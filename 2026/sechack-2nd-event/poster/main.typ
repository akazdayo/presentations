    ...
    [
      #section(
        ...
        [
          ...
          そこでNixでは、あらかじめビルドされた成果物を取得できるBinary Cacheが利用される。
        ],
      )
      ...
      #section(
        ...
        [
          ...
          - 結果として、利用実績のある大規模なCacheへ信頼が集中する
        ],
      )
      ...
      #section(
        [なぜ成果物の一致を確かめるのか],
        [
          利用者が確かめたいのは、指定した入力に対応する成果物である。たとえばGoogle Chromeを導入しようとしたのに、Chromiumへすり替えられるのは困る。

          本研究はマルウェア判定ではなく、指定した入力から得られるはずの成果物との一致を確かめる。マルウェア検出はVirusTotalなどの専用サービスに任せる方が効率的で現実的な可能性が高い。
        ],
      )
    ],
    [
